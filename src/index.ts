 ...
 * inject the continuation as a user message so the agent can respond to it.
 */
async function checkPendingRestart(): Promise<void> {
  ...
  try {
    ...
    if (data.chatJid && data.continuation_prompt) {
      // Inject continuation_prompt as a user message so the agent responds to it
      const syntheticMessage: NewMessage = {
        id: `restart-continuation-${Date.now()}`,
        chat_jid: data.chatJid,
        sender: data.chatJid, // From the user/group
        sender_name: 'User',
      ...
      logger.info(
        ...
        'Injected restart continuation as user message',
      );

      // Trigger agent to process the message
